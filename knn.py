...
class KNN:
    ...
    def compute_distances_no_loops(self, X):
        ...
        X = np.tile(X, [1, num_train])
        train_X = np.reshape(self.train_X, (1, self.train_X.shape[1]*num_train))
        X = X - train_X
        dists = np.sum(np.abs(np.reshape(X, (num_test*num_train, self.train_X.shape[1]))), axis = 1)
        return np.reshape(dists, (num_test, num_train))
